...
module.exports = function () {
	...
	_helpers.personUrl = function (personSlug, options) {
	...
	* */
	...
	_helpers.paginationNavigation = function (pages, currentPage, totalPages, options) {
		...
		_.each(pages, function (page, ctr) {
			...
			html += '<a' + liClass + 'href="' + pageUrl + '">' + pageText + '</a>';
		});
		return html;
	};
	...
	_helpers.uneven = function (index) {
		if (index % 2 === 1) {
			return true;
		} else {
			return false;
	...
	_helpers.pageScript = function (currentSection) {
	...
	_helpers.postBriefCutter = function (data) {
		if (data && data.length > 300) {
			return data.slice(0, 301);
		}
	};
	
	_helpers.tagsList = function (tags) {
		if (tags && tags.length !== 0) {
			var result = '';
			for(var i=0; i<tags.length; i++){
				result += '<a href="'+ tags[i].key + '">' + tags[i].value + '</a>, ';
			}
	...
	_helpers.__ = function () {
		return keystone.t.apply(this, arguments);
	};
